...
class Encoder(nn.Module):
	def __init__(self):
		...
		self.demention=3
		self.encoder = nn.Sequential(nn.Linear(1400*self.demention, 786),nn.PReLU(),nn.Linear(786, 512),nn.PReLU(),nn.Linear(512, 256),nn.PReLU(),nn.Linear(256, 60))
...
class Decoder(nn.Module):
	def __init__(self):
		...
		self.demention = 3
		self.decoder = nn.Sequential(nn.Linear(60, 256),nn.PReLU(),nn.Linear(256, 512),nn.PReLU(),nn.Linear(512, 786),nn.PReLU(),nn.Linear(786, 1400*self.demention))
	def forward(self, x):
		x = self.decoder(x)
		return x
